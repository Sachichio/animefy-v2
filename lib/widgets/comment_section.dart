...
class _CommentSectionState extends State<CommentSection> {
  ...
  String loginUserRole = "user"; // role user yang login
  ...
  Future<void> loadComments() async {
    ...
    try {
      ...
      for (var c in data) {
        ...
        if (uid != null && !usernameCache.containsKey(uid)) {
          final username = await UserService.getUsernameById(uid);
          usernameCache[uid] = username ?? "User $uid";
        }

        if (uid != null && !userRoleCache.containsKey(uid)) {
  ...
  /// EDIT COMMENT (tidak update timestamp!)
        ...
        children: [
          ...
          ElevatedButton(
            ...
            style: ElevatedButton.styleFrom(
              ...
              foregroundColor:
                  isDark ? Colors.yellow.shade700 : Colors.white,
            ),
  ...
  Widget _buildCommentList(bool isDark) {
    return ListView.builder(
      ...
      itemBuilder: (context, i) {
        ...
        final username = usernameCache[c['userId']] ?? "Unknown";
        final userRole = userRoleCache[c['userId']] ?? "user";

        final time = RelativeTime.format(c['timestamp'] ?? "");

        final bool isOwner = widget.userId == c['userId'];
        ...
        return Container(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              /// HEADER KOMENTAR: USERNAME + BADGE ADMIN + TIME + EDITED
              Row(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Expanded(
